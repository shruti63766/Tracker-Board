...
Admins (including the branch manager account) can use **Add target** beside a member to assign additional named targets for the selected month. Each target has its own Save button and its own progress calculation. Employees select the relevant target when adding or editing a recovery entry, so charts, the leaderboard, summaries, and exports show every assignment separately. Amounts are displayed as plain Indian-formatted numbers without a currency symbol.

For an existing Supabase installation, run `supabase/multiple-targets.sql` in the SQL editor **before deploying the updated frontend**. This preserves existing records, links historical entries automatically when only one target matches, updates the snapshot and target-level progress view, and installs the target-aware entry RPCs. Historical entries with more than one possible target remain unassigned until the employee edits them. New installations use the updated `supabase/schema.sql`.
